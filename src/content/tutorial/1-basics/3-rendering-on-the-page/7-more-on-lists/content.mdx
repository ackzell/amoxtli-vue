...
We now have the following basic structure for the messages in a chat app:
...
We can loop through the `chatMessages` array with a `v-for`. [We know that](/basics/rendering-on-the-page/rendering-lists), but now that we also want the reactions rendered, we'll have to loop through those as well.

Check this out:

```file:/App.vue title="App.vue" collapse={1-29, 40-103} {"1. Loop through the messages":30-31} {"2. Loop through the reactions":33-34}
```

You can write them as nested loops (I don't know there's a limit on how many levels deep you can nest to be honest) and that the <AvTooltipTrigger id="nested-scopes">_scope_ of the outer loop is still accessible to the inner one</AvTooltipTrigger>, just like in JS functions!
...
```file:/App.vue title="App.vue" collapse={1-29, 40-103} /:key="[^"]+"/
```
...
<AvTooltipContent id="nested-scopes">
:::info
