...
import { ToggleLessonStateButton } from '@/components/SolveLessonButton.tsx'
...
Anyhow, let's create a simple structure that will hold some data about the members:
...
```file:/App.vue title="App.vue"  /theBeatles/ collapse={1-23,28-58}
```

Well, we do see them on the page. But it just shows the entire array "as is", like: 

<div className='bg-bgr dark:bg-bgr-dark p-4 border border-tk-border-secondary'>
...
Fortunately, HTML has specific tags  for this purpose that also help to present the data more clearly: `<ol>`, `<ul>` and `<li>`. And we want to write <AvTooltipTrigger id="semantic-markup">semantic markup</AvTooltipTrigger> as much as we can, right? right?? enter:

# `v-for`

This is how we list our musical legends using `v-for`:

```file:/App.vue  /v-for="[^"]+"/ collapse={1-26, 38-58}
```
...
<AvTooltipContent id="semantic-markup">
:::info
